...
class Node(object):
    def __init__(self, curr_state, parent, action, pos, solved, g, u):
        ...
        if u is not None:
            ...
            self.manhattanDistance = manhattan(self.candidate, self.solved, len(curr_state[0])) * 2
            self.linearConflict = 2 * linear_conflicts(self.candidate, self.solved, len(curr_state[0]))
            self.heuristic = self.manhattanDistance + self.linearConflict
